...
## Bayesian Marketing Mix Modeling (MMM) for Pay-TV and Streaming
...
**Why aggregate-level modeling:** Since the only available data is aggregate weekly subscriber counts with no individual customer identifiers or touchpoint history, the model needs to operate at the aggregate level. Marketing Mix Modeling (MMM) is built exactly for this: it works on weekly totals, decomposing total subscriber volume into contributions from each driver using time-series regression without needing any individual-level data.
...
- *Priors:* Marketing channels get HalfNormal priors, meaning the effect must be ≥ 0 because advertising cannot make subscribers go down. Competitor promotions get a Normal prior centered negative, as competitor promos are expected to suppress acquisition. Macro index gets a symmetric Normal prior since the effect direction is genuinely ambiguous
- *Why Bayesian over regular regression:* Instead of "the promo added 796 subscribers" (false precision), we get "we are 90% confident the promo added between 685 and 1,103 subscribers." That honest uncertainty that changes how you think about investment risk
...
Marketing measurement models come in different forms depending on what data is available. Some approaches work at the individual customer level, tracking each person's ad exposures, clicks, and the moment of conversion. These are powerful when individual-level data exists.

**Marketing Mix Modeling (MMM)** takes a different approach. It operates entirely at the aggregate level, requiring only time-series totals: weekly subscriber counts, weekly spend per channel, and contextual signals. No individual-level data is needed or used.

Since the only available data here is aggregate weekly subscriber counts from web analytics, with no individual customer identifiers or clickstream data, MMM is the natural fit. It's designed precisely for situations where you have aggregate outcomes and want to understand what drove them.
...
|---|---|---|
| TV | 0.65 | Slow decay; brand awareness is durable and effects linger weeks |
...
| `β_macro` | Normal(0, 100) | Effect direction ambiguous; symmetric prior lets data decide |
...
With the model fitted, incrementality is computed by comparing the actual campaign window outcome against a **counterfactual**, which is what the model predicts subscriber volume would have been with the promotion removed:
...
| 90% Credible Interval | [685 to 1,103] subscribers |
